...
import static utils.Constants.EnemyConstants.SHARDSOUL_SLAYER;
import static utils.Constants.EnemyConstants.ShardsoulSlayer.*;

public final class ShardsoulSlayer extends  Enemy
{
    //AttackBox
    ...
    public ShardsoulSlayer(double x, double y, int tilesInHeight)
    {
        super(x, y, WIDTH, HEIGHT, tilesInHeight, SHARDSOUL_SLAYER);
        state = IDLE;
        walkSpeed = 1.5 * SCALE;
        initHitBox(24, 31, 0, 0);
        initAttackBox();
        knockbackFactor = 5;
        doesKnockBack = true;
    }
    ...
    {
        attackBox = new Rectangle2D.Double(x, y, (int)(48 * SCALE), (int)(31 * SCALE));
    ...
    public void update(int[][] levelData, Player player)
    {
        ...
        if(state == DEATH)
        {
            if(!death)
            {
                playing.getGame().getAudioPlayer().playEffect(AudioPlayer.SHARDSOUL_SLAYER_DEATH);
                death = true;
            }
        }
    }
    ...
    private void updateAttackBox()
    {
        if(flipW() == -1)
        {
            attackBox.x = hitBox.x + attackBoxOffsetX - (18 * SCALE);
        }
        else
        {
    ...
    private void updateBehaviour(int[][] levelData, Player player)
    {
        ...
        else
        {
            switch(state)
            {
                case IDLE ->
                {
                    if (canSeePlayer(levelData, player, 5))
                    {
                ...
                case WALK ->
                {
                    if (idleTimeCounter > 0 && !canSeePlayer(levelData, player, 5))
                    {
                        newState(IDLE);
                    }
                    else
                ...
                case ATTACK ->
                {
                    ...
                    {
                        playing.getGame().getAudioPlayer().playEffect(AudioPlayer.SHARDSOUL_SLAYER_ATTACK);
